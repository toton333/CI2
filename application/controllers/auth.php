...
class Auth extends CI_Controller {
	...
	function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				//redirecting to the url which is written in referrer.php
				$redirect = read_file('application/referrer/referrer.php');
               	write_file('application/referrer/referrer.php', "");
               	redirect($redirect);
			}
			else
			{
		...
		else
		{
			...
			//writing referrer url into a file
			   $referrer = $this->agent->referrer();

               	if ($referrer != "http://localhost/CIpractice2/auth/login" && $referrer != "http://localhost/CIpractice2/auth" && !empty($referrer))
               	{
               		write_file('application/referrer/referrer.php', $referrer);
               	}
	...
	function activate($id, $code=false)
	{
		...
		if ($activation)
		{
			...
			$this->session->set_flashdata('message', $this->ion_auth->messages());
			redirect("/", 'refresh'); //custom redirecting to homepage instead of login page cause of login referrer
		}
		else
		{
	...
	public function self_update($id)
	 {
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() === TRUE)
			{
				...
				//custom if else statement to show error message if duplicate username or email

				if ($this->ion_auth->update($user->id, $data))
				{
					
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				redirect('auth/self_update/'.$user->id, 'refresh');
				}
				else
				{
					$this->session->set_flashdata('message', $this->ion_auth->errors());
        ...
        $this->load->view('templates/header', $this->data);
		$this->_render_page('auth/self_update', $this->data);
